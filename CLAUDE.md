...
- `docs/SPEC_CONNECTEURS_ET_SUJETS.md` — ce qu'est un « sujet » chez GitHub, Notion, Linear, Etsy et Shopify, et ce que coûte de les brancher. Notion et Linear sont **implémentés** (migration `0028` appliquée en base de dev), non testés avec de vraies clés. Etsy et Shopify sont bloqués derrière une validation manuelle de leur plateforme — démarches détaillées dans `docs/DEMANDES_ETSY_SHOPIFY.md`, à mener en parallèle du code.
