/**
 * Lot 1 (docs/SPEC_PROMPT_GENERATION_TECH.md §7) — "après" prompt v2.
 *
 * Rejoue EXACTEMENT les 5 mêmes briefs que scripts/baseline-lot0-run.ts (mêmes ids en base, même
 * ordre), mais avec le prompt de génération mis à jour par le Lot 1 (§3.1-3.3 : SCRIPT_SYSTEM_PROMPT
 * v2, buildScriptUserMessage v2 en blocs balisés, constante partagée EDITORIAL_WRITING_RULES) —
 * toujours forcé sur OpenAI, zéro migration DB (le champ "concept" et l'audience n'existent pas
 * encore, cf. Lots 2/4).
 *
 * N'écrit RIEN en base, même mécanique que le run Lot 0.
 *
 * Usage : node_modules/.bin/tsx scripts/baseline-lot1-run.ts
...
const OUT_DIR = path.join(process.cwd(), "docs", "baseline", "lot1-openai");
...
async function main() {
  ...
  const summary: string[] = [
    "# Baseline Lot 1 — sorties brutes (après prompt v2, §3.1-3.3)",
    "",
