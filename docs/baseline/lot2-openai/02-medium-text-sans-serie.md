...
=== BRIEF ===
Produit/Sujet concerné : PPO HotlineMiami
...
{
  "contentType": "text",
  "concept": "Je m’adresse aux développeurs indie et aux créateurs qui entraînent un agent de jeu : montrer, à partir du cas très concret de PPO HotlineMiami, pourquoi il faut tester une hypothèse avec un changement isolé et une métrique adaptée plutôt que prolonger aveuglément un run. Ils doivent retenir qu’un score global peut masquer un blocage précis et apprendre à diagnostiquer la cause avant de relancer.",
  "title": "Le coin à 60 pixels",
  "caption": "J’avais une hypothèse simple : l’agent PPO avait seulement besoin de plus de temps pour apprendre le détour de `room_stage=2`.\n\nJ’ai donc repris une configuration entièrement fixe — mêmes spawns, même sortie — et ajouté 15 millions de pas d’entraînement.\n\nRésultat : 0% de succès sur les 600 points d’évaluation.\n\nLe combat, lui, oscillait violemment : jusqu’à 98% de salles nettoyées à un moment, puis 12%. Ce n’était donc pas uniquement un problème de navigation.\n\nLe diagnostic a changé quand j’ai arrêté de regarder seulement `success_rate` :\n- avec le reward discret, l’agent bloquait vers x≈526 ;\n- avec un reward de rapprochement continu, il atteignait systématiquement 60 à 70 px de la sortie ;\n- après 5 millions de pas supplémentaires, il restait au même endroit, collé au bord du mur miroir ;\n- en augmentant temporairement l’exploration avec `ent_coef=0.01`, certains épisodes ont enfin franchi ce coin — mais le combat s’est dégradé.\n\nLa leçon n’est pas « entraînez plus longtemps ».\n\nC’est : mesurez la distance au point de blocage, observez le comportement, puis changez une seule variable. Ici, le problème n’était pas un manque de budget : c’était une politique devenue trop confiante pour sortir d’un mauvais choix local.\n\nDans vos agents RL, quelle métrique avez-vous ajoutée pour voir ce que le score global cachait ?",
  "hashtags": [
  ...
  "usedExcerpts": [
    "15 000 000 pas supplémentaires sur la version la plus simple possible de la tâche n'ont produit aucun signe de convergence.",
    "success_rate reste EXACTEMENT à 0,00 sur les 600 points, du premier au dernier pas des 15M",
    "room_cleared_rate : 0,12 à un point, 0,98 au point suivant quelques millions de pas plus loin, puis retombe",
    "distance minimale au centre de la sortie : 64,2 à 70,3px - STRICTEMENT IDENTIQUE (à quelques px près) au run 28",
    "l'agent se colle contre le mur en haut du détour et ne bouge plus",
    "Distance minimale au centre de la sortie sur le MEILLEUR épisode : 42,4px - nettement mieux que le plancher dur de 60-70px observé aux runs 28/28-suite",
    "ent_coef=0.01 fait effectivement remonter l'exploration, au coût attendu de précision en combat"
  ],
  "hookText": "15 millions de pas supplémentaires n’ont pas rapproché l’agent de la sortie d’un seul pixel significatif."
}
```
