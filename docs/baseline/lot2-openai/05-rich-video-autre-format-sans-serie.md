# Brief 05-rich-video-autre-format-sans-serie

> Même matière riche que le brief 1 (MyTwin Leaderboard) mais YouTube, pas de série — compare le rendu du même corpus sur un autre format
...
=== BRIEF ===
Produit/Sujet concerné : MyTwin Leaderboard
Plateforme cible : youtube. YouTube (Shorts ou format long) : hook visuel/verbal dès la première seconde, structure narrative plus développée qu'un short TikTok.
...
{
  "contentType": "video",
  "concept": "Je lance un défi aux contributeurs ML du lab : faire tester par des humains les APIs issues des deux challenges ouverts, « mammography classification » et « mammography segmentation ». La vidéo s’adresse aux contributeurs techniques et doit leur donner envie de participer à ces tests, en montrant que le résultat ne repose pas seulement sur le code ou un score IA, mais sur ce que l’API renvoie réellement quand on lui donne un fichier.",
  "title": "Le test humain des deux APIs mammographie",
  "caption": "J’ai commencé à reprendre le contact avec certains des 60 contributeurs de lab : ceux qui font du ML afin de les lancer sur les 2 challenges ouverts : mammography classification et mammography segmentation.\n\nLe défi que je leur propose tient en quelques étapes :\n- déposer un fichier sur une API ML exposée ;\n- regarder sa réponse réelle ;\n- voter « Fonctionne » ou « Défectueux ».\n\nL’idée n’est pas de remplacer l’évaluation du code. C’est de vérifier une chose très concrète : est-ce que le modèle déployé répond effectivement quand on l’appelle ?\n\nSi tu contribues sur l’un de ces challenges, participe aux tests et partage ton verdict dans le challenge.",
  "hashtags": [
    "#MachineLearning",
    "#MLOps",
    "#MedicalAI",
  ...
  "usedExcerpts": [
    "J'ai commencé à reprendre le contact avec certains des 60 contributeurs de lab : ceux qui font du ML afin de les lancer sur les 2 challenges ouverts : mammography classification et mammography segmentation",
    "`type: 'validation'` challenges let contributors manually test whether a submitted ML API packaging actually works — drop a file, call the live deployed endpoint, see the raw output, then vote **Fonctionne** or **Défectueux**.",
    "This is deliberately **not** an automated grader. It doesn't touch `evaluation_status`, `evaluation`, or `globalScore` on the source contribution at all — it's a fully separate mechanism with its own CP pool, for a fully separate purpose (does it work?, not how good is the code?)."
  ],
  "hookVisual": "Je pose deux étiquettes manuscrites — « classification » et « segmentation » — à côté d’un fichier de test, puis je fais glisser le fichier vers une zone de dépôt visible à l’écran.",
  "hookText": "Le code peut passer. Mais est-ce que l’API répond vraiment ?",
  "hookAudio": "« J’ai commencé à reprendre contact avec certains des 60 contributeurs du lab. Cette fois, le défi est simple : prendre une API ML en ligne, lui envoyer un fichier, puis décider si elle fonctionne ou si elle est défectueuse. »",
  "storyboard": [
    {
      "planNumber": 1,
      "description": "Plan rapproché : je pose sur une table les deux étiquettes « mammography classification » et « mammography segmentation », puis un fichier de test entre les deux."
    },
    {
      "planNumber": 2,
      "description": "Face caméra : je dis que j’ai commencé à reprendre contact avec certains des 60 contributeurs du lab, ceux qui font du ML."
    },
    {
      "planNumber": 3,
      "description": "Capture d’écran de la page du challenge : je montre les deux challenges ouverts, « mammography classification » et « mammography segmentation », sans ajouter d’information non visible."
    },
    {
      "planNumber": 4,
      "description": "Capture d’écran ou plan de démonstration : je dépose un fichier sur une cible d’API exposée et je montre que l’appel passe par la plateforme."
    },
    {
      "planNumber": 5,
      "description": "Plan écran : je montre la réponse brute renvoyée par l’API, puis j’affiche successivement les verdicts « Fonctionne » et « Défectueux » comme les deux choix possibles."
    },
    {
      "planNumber": 6,
      "description": "Face caméra : je résume la règle du défi — on ne juge pas la qualité du code, on vérifie ce que renvoie réellement l’endpoint déployé — puis je pointe vers les deux challenges ML."
    }
  ],
  "soundRecommendation": "Voix off naturelle, avec sons réels de clavier et de dépôt de fichier ; ajouter une montée musicale discrète au moment où la réponse de l’API apparaît, puis couper le son avant le verdict pour le rendre lisible."
}
```
