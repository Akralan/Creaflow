...
{
  "contentType": "text",
  "title": "Le challenge PPO : apprendre à franchir le dernier coin",
  "caption": "J’ai lancé un challenge simple à mon agent PPO sur MarvelousDev : nettoyer la salle, suivre le chemin vers la sortie et franchir un détour imposé par un mur miroir.\n\nLes règles étaient les mêmes, avec un seul changement important : l’agent devait apprendre à naviguer après le combat, sans sacrifier sa précision de tir.\n\nPremière tentative : il nettoie presque parfaitement la salle, puis se bloque toujours au même endroit — collé contre le mur, en haut du détour. Il atteint systématiquement une zone située à 60–70 px de la sortie, mais ne la rejoint jamais.\n\nJ’ai alors activé une récompense de rapprochement continue, calculée en pixels plutôt qu’au seul franchissement d’une case. Résultat : l’agent s’approche davantage, mais le blocage reste identique.\n\nJ’ai ensuite ajouté un coefficient d’entropie pour restaurer de l’exploration. Cette fois, le comportement change : certaines parties franchissent enfin le coin bloquant et atteignent la bonne cellule. Mais le combat se dégrade en parallèle.\n\nLe résultat du challenge n’est donc pas “PPO a échoué”. Il est plus précis : une seule politique doit ici résoudre deux problèmes qui demandent des comportements opposés — viser avec précision et explorer un passage qu’elle n’a jamais réussi à franchir.\n\nLa prochaine piste logique est de séparer ces rôles : une politique combat, puis une politique navigation une fois tous les ennemis éliminés.\n\nC’est exactement le type de détail qui change un simple réglage de reward en vrai problème d’architecture RL.",
  "hashtags": [
    "#GameDev",
  ...
  "usedExcerpts": [
    "**Distance minimale au centre de la sortie jamais atteinte par épisode : 60,4px (meilleur cas) à 70,5px (pire des meilleurs cas)**",
    "**l'agent se colle contre le mur en haut du détour et ne bouge plus**",
    "**Distance minimale au centre de la sortie sur le MEILLEUR épisode : 42,4px**",
    "**une partie des épisodes (pas encore la majorité) franchit maintenant le coin qui bloquait tous les runs précédents**",
    "**ent_coef constant maintient un bruit d'exploration permanent qui empêche à la fois la consolidation du franchissement (utile) et la stabilisation du combat (coûteux)**",
    "**Idée : entraîner deux PPO distincts plutôt qu'une seule politique qui doit faire les deux**"
  ],
  "hookText": "J’ai donné 5 millions de pas de plus à mon agent PPO : il n’a pas avancé d’un pixel sur le vrai problème."
}
```
