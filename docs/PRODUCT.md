...
| **Génération de Contenu** | Scripts vidéo/visuel/texte, storyboards, légendes, hashtags, angles anti-répétition, séries récurrentes, photo mise en scène (Nano Banana) et **maquette du post visuel** composée par l'agent en HTML par-dessus l'image (ou sans), éditable à la main et par instruction, exportée en PNG (`docs/SPEC_DESIGN_HTML_SUR_IMAGE.md`) ; **format choisi à la génération** — image, carrousel de N slides, ou **animation** de 5 à 15 s dont les textes entrent et sortent, aperçu dans l'éditeur, export MP4 au clic (`docs/SPEC_FORMAT_VISUEL_ET_ANIMATION.md`) | Voix-off et musique sur les animations, coloration syntaxique des extraits de code, carrousel PDF pour LinkedIn |
...
11. ~~Format du post visuel (image, carrousel de N, animation)~~ — livré (`docs/SPEC_FORMAT_VISUEL_ET_ANIMATION.md`) : le format se choisit à la génération, l'animation est une slide dont les textes entrent et sortent sur 5 à 15 s, exportée en MP4 au clic. Restent le son et le repli pour les navigateurs sans WebCodecs.
