...
Le sélecteur de dépôts (`RepoPicker`) n'est pas réservé à l'onboarding : paramètres > Sujets le propose aussi (`variant="settings"`, encart « Ajouter depuis GitHub » affiché si `GET /api/profile` renvoie `githubConnected: true`) pour connecter de nouveaux dépôts après coup. `GET /api/github/repos` marque chaque dépôt d'un `alreadyConnected` (existence d'une `material_sources` de type `github_repo` sur son `externalId`) — l'UI grise ces dépôts au lieu de laisser la connexion doublonner le sujet et violer l'unicité de `material_sources`.
