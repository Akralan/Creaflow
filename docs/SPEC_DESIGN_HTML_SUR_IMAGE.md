...
> **Statut : implémenté (Lots 0 à 4) sur `feature/design-html-sur-image`, migration `0030` générée (à appliquer avec `npm run db:migrate`), non testé avec une vraie clé LLM ni dans un navigateur réel.** Écarts assumés par rapport au cadrage : les polices sont chargées par `next/font/google` (variables CSS, `src/app/layout.tsx`) plutôt qu'auto-hébergées dans `public/fonts` — même résultat pour l'export (fichiers same-origin), sans binaires dans le dépôt ; le HTML stocké garde les noms lisibles (`font-family:Inter`), traduits en variables au rendu (`toRenderableHtml`). Le parseur HTML de la liste blanche est maison (aucune dépendance), strict, et resérialise sous forme canonique. La vignette dans Calendrier/Performance (§6) n'est pas faite. **À vérifier manuellement en priorité** : l'export PNG (`html-to-image`) avec polices et image de base, sur Chrome et Firefox. Prolonge `docs/SPEC_RESSOURCES_VISUELLES.md`, qui a livré la bibliothèque d'images de marque et la photo mise en scène (Nano Banana). Ce document cadre l'étape d'après : la **maquette** du post.
>
...
- **Propriétés CSS** : mise en page (`display`, `flex-*`, `align-*`, `justify-*`, `gap`, `position`, `top/left/right/bottom`, `width/height`, `padding`, `margin`, `overflow`, `z-index`) ; typographie (`font-family` restreinte à la liste, `font-size`, `font-weight`, `line-height`, `letter-spacing`, `text-align`, `text-transform`, `white-space`, `color`) ; décoration (`background`, `background-color`, `background-image` limité aux gradients (une image de fond passe par `<img src="{{BASE_IMAGE}}">`), `background-size/position`, `border`, `border-radius`, `box-shadow`, `opacity`, `transform`). Toute valeur contenant `url(`, `expression(`, `@import` ou un `;` mal formé est rejetée.
