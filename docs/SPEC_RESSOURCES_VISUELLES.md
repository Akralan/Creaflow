...
4. ~~**Modèle d'embedding**~~ **Tranché : `text-embedding-004`, dimension 768** (`GEMINI_EMBEDDING_MODEL`, `src/lib/llm/embeddings.ts`), cohérent avec `brandAssets.embedding` (`vector(768)`, `src/db/schema.ts`).
   **⚠ BUG CONSTATÉ EN ENVIRONNEMENT RÉEL (2026-08-25), NON CORRIGÉ** — première vérification réelle de ce cadrage (baseline `docs/baseline/lot{0,1,2}-openai/`, brief `03-sparse-visual-serie`, contentType `visual`) : `embedText()` échoue systématiquement avec `404 models/text-embedding-004 is not found for API version v1beta, or is not supported for embedContent`. `findBestBrandAssetForScript` (`brandAssetService.ts`) appelle `embedText()` (toujours via Gemini, **indépendamment de `LLM_PROVIDER`**) pour tout `contentType="visual"` dès qu'un `BrandAsset` existe — donc **toute génération de contenu visuel est cassée en l'état**, pas seulement ce test. Piste : le modèle a probablement été retiré/renommé côté API Gemini depuis que cette décision a été tranchée ; vérifier `ModelService.ListModels` pour le nom actuel avant de corriger `GEMINI_EMBEDDING_MODEL`. Hors périmètre du chantier qui l'a découvert (`docs/SPEC_PROMPT_GENERATION_TECH.md`) — à corriger séparément.
