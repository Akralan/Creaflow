...
> **Statut : implémenté (Lots 0 à 4) sur `feature/apprentissage-style`, migration `0029` générée (à appliquer avec `npm run db:migrate`), non testé avec une vraie clé LLM.** Écart assumé par rapport au cadrage : quand aucun profil de style n'existe encore, la passe **écrit directement** au lieu de proposer (rien à protéger, et le callback OAuth ne peut pas attendre une validation) ; la proposition à valider ne s'applique qu'à partir du second passage. Prolonge `docs/SPEC_MATIERE_EDITEUR.md` §4.7 (« sous-produit stratégique — la donnée de voix »), qui avait tranché de **stocker** le gisement premier jet / version finale sans l'exploiter. Ce document cadre l'exploitation. Point 9 de `docs/PRODUCT.md` §6.
>
